...
export default function GraphVisualization({ data, onNodeClick }: GraphVisualizationProps) {
  ...
  const drawNode = useCallback((node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    const fontSize = (node.type === 'vibe' || node.type === 'artist') ? 14/globalScale : 12/globalScale;
    ...
    switch (node.type) {
      ...
      case 'artist':
        ...
        ctx.rect(node.x - squareSize/2, node.y - squareSize/2, squareSize, squareSize);
        break;
    }
  ...
  const graphData = useCallback((): GraphData => {
    ...
    data.songs.forEach(song => {
      const clusterTarget = viewMode === 'vibe' 
        ? (song.vibes[0] ? `vibe-${song.vibes[0].id}` : undefined)
